...
export default class TracerBullet extends GameObject {
  ...
  update(_deltaTime: number) {
    ...
    if (this.gameObject.position.y >= this.game.canvas.canvasHeight - this.gameObject.height) {
      const diffY = this.gameObject.position.y - this.game.player.gameObject.position.y - 16;
      const diffX = this.gameObject.position.x - this.game.player.gameObject.position.x - 16;
      const distance = Math.sqrt(
        (this.gameObject.position.x - this.game.player.gameObject.position.x) ** 2 +
          (this.gameObject.position.y - this.game.player.gameObject.position.y) ** 2,
      );
