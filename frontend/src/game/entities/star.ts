...
export default class Star extends GameObject {
  ...
  draw(context: any) {
    ...
    context.beginPath();
    context.moveTo(this.gameObject.position.x + this.gameObject.width / 2, this.gameObject.position.y);
    context.lineTo(
    ...
    context.lineTo(this.gameObject.position.x, this.gameObject.position.y + this.gameObject.height / 2);
    context.stroke();
    context.fill();
    context.globalAlpha = Math.max(0, (this.MAX_PULSE - this.pulseTimer) / this.MAX_PULSE);
    context.strokeStyle = COLOR.GOLD;
    context.beginPath();
    context.arc(
      this.gameObject.position.x + this.gameObject.width / 2,
