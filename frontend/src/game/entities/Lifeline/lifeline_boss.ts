...
import LifelineBullet from 'game/entities/Lifeline/lifeline_bullet.ts';
import BasicBullet from 'game/entities/Basic/basic_bullet.ts';
...
export default class LifelineBoss extends GameObject {
  game: Game;
  awaken: boolean;
  bullet_timer: number;
  ...
  fireBullets() {
    ...
    if (this.awaken && this.bullet_timer % 20 === 0) {
      ...
      let BulletClass = getRandomNumber(0, 20) > 1 ? BasicBullet : LifelineBullet;
      ...
      BulletClass = getRandomNumber(0, 20) >= 1 ? BasicBullet : LifelineBullet;
      this.game.gameObjects.push(
        new BulletClass({
          game: this.game,
          position: { x: origin_x, y: origin_y },
