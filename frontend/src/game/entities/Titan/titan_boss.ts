...
import TitanBullet from './titan_bullet.ts';
...
export default class TitanBoss extends GameObject {
  ...
  draw(context: any) {
    ...
    gradient.addColorStop(Math.min(1 * (this.gameObject.width / 300), 1), COLOR.DARK_BLUE);
  ...
  update(_deltaTime: number) {
    ...
    this.gameObject.position.x =
      (1 + Math.cos(this.angle)) * (this.game.canvas.canvasWidth / 2 - this.gameObject.width / 2) + 20;
    this.angle += 0.02;
