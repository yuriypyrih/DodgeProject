  ...
  update(_deltaTime: number) {
    ...
    this.game.particleObjects.push(
      new Trail({
        ...
        width: this.gameObject.width,
        height: this.gameObject.height,
        life: Math.max(0.7 * ((this.MAX_SIZE - this.gameObject.width) / this.MAX_SIZE), 0.2),
        minus: 0.02,
        game: this.game,
      }),
    );
    ...
    if (this.gameObject.position.y <= 0 && this.gameObject.velY < 0) this.gameObject.velY *= -1;
    if (
    ...
    if (this.gameObject.position.x <= 0 && this.gameObject.velX < 0) this.gameObject.velX *= -1;
    if (this.gameObject.position.x >= this.game.canvas.canvasWidth - this.gameObject.width && this.gameObject.velX > 0)
