...
export default class TitanBullet extends GameObject {
  ...
  update(_deltaTime: number) {
    ...
    const diffY = Math.ceil(this.gameObject.position.y - (this.boss.gameObject.position.y + 5));
    const diffX = Math.ceil(this.gameObject.position.x - (this.boss.gameObject.position.x + 5));
