...
import RadioBullet from 'game/entities/Radioactive/radio_bullet.ts';
import ShadowBullet from 'game/entities/Shadow/shadow_bullet.ts';
...
export default class RadioBoss extends GameObject {
  game: Game;
  awaken: boolean;
  bullet_timer: number;
  awakening_timer: number;

  constructor({ game, position, velX = 0, velY = 0.3 }: TProps) {
    super({
      id: ENTITY_ID.BOSS,
      width: 50,
      height: 50,
