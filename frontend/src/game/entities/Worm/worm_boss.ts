...
import WormBullet from './worm_bullet.ts';
...
export default class WormBoss extends GameObject {
  ...
  constructor({ game, position, velX = 0, velY = 0.3, skipAwakening = false }: WormBossProps) {
    super({
      id: ENTITY_ID.BOSS,
      width: 50,
      height: 50,
  ...
  update(_deltaTime: number) {
    ...
    if (Math.abs(this.gameObject.velY) === this.MAX_SPEED || Math.abs(this.gameObject.velX) === this.MAX_SPEED)
