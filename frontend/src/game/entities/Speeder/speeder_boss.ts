...
import SpeederBullet from './speeder_bullet.ts';
...
export default class SpeederBoss extends GameObject {
    ...
    if (this.awaken && this.bullet_timer % 40 === 0) {
      ...
      this.game.gameObjects.push(
        new SpeederBullet({
          ...
          velY: Math.max(1, 5 * ((this.MAX_SPEED - Math.abs(this.gameObject.velX)) / this.MAX_SPEED)),
        }),
      );
    }
  }
  ...
  update(_deltaTime: number) {
    ...
    if (
      ...
      if (this.gameObject.velX < this.MAX_SPEED && this.gameObject.velX > -this.MAX_SPEED) {
