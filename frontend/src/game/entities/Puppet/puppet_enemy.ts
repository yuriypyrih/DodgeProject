...
export default class PuppetEnemy extends GameObject {
  game: Game;
  ...
  update(_deltaTime: number) {
    ...
    const playerObj = this.game.player.gameObject;
    if (playerObj.velX > 0) {
      this.gameObject.velX = Math.abs(this.gameObject.velX);
    }
    if (playerObj.velX < 0) {
      this.gameObject.velX = -Math.abs(this.gameObject.velX);
    }
    if (playerObj.velY > 0) {
      this.gameObject.velY = Math.abs(this.gameObject.velY);
    }
    if (playerObj.velY < 0) {
      this.gameObject.velY = -Math.abs(this.gameObject.velY);
    }
