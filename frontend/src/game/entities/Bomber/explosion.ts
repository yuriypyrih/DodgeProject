...
export default class Explosion extends GameObject {
  ...
  draw(context: any) {
    ...
    const gradient = context.createRadialGradient(
      ...
      Math.min(this.MAX_EXPLOSION_RAD - 40, Math.max(this.explosionTimer - 40, 0)),
      this.gameObject.position.x + 10,
      this.gameObject.position.y + 10,
  ...
  update(_deltaTime: number) {
    this.gameObject.width = Math.ceil(this.MAX_DMG_RADIUS * (this.explosionTimer / this.MAX_EXPLOSION_RAD));
    this.gameObject.height = Math.ceil(this.MAX_DMG_RADIUS * (this.explosionTimer / this.MAX_EXPLOSION_RAD));
    this.explosionTimer += Math.max(this.explosionAddition, 0);
    this.explosionAddition =
      (this.EXPLOSION_ADD * (this.MAX_EXPLOSION_RAD - this.explosionTimer)) / this.MAX_EXPLOSION_RAD;
