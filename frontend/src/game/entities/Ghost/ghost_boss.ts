...
import GhostBullet from './ghost_bullet.ts';
...
export default class GhostBoss extends GameObject {
  ...
  constructor({ game, position, velX = 0, velY = 0.3, skipAwakening = false }: GhostBossProps) {
    super({
      id: ENTITY_ID.BOSS,
      width: 50,
      height: 50,
  ...
  fireBullets() {
    this.bullet_timer++;
    if (this.awaken && this.bullet_timer % 15 === 0) {
