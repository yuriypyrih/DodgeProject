...
import VoidEnemy from 'game/entities/Voidborn/void_enemy.ts';
...
export const level32Stars: Stars = [6, 40, 62];

const levelStars = level32Stars;

export const getLevel32 = (game: Game): null => {
  if (game.spawner.executionSequence === 0) {
    if (game.spawner.roundTimer === sec(0.1)) {
      store.dispatch(playText(['LEVEL 32', 'Tether']));
    } else if (game.spawner.roundTimer === sec(1.5)) {
      game.gameObjects.push(new TetherEnemy({ game, position: { x: 1, y: 40 } }));
    ...
    if (game.spawner.roundTimer === sec(levelStars[0] + 1)) {
      game.gameObjects.push(new VoidEnemy({ game, position: { x: 40, y: 10 } }));
    } else if (game.spawner.roundTimer === sec(levelStars[0] + 2.2)) {
