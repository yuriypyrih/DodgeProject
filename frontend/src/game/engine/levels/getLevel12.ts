...
import MimicBoss from '../../entities/Mimic/mimic_boss.ts';
...
export const getLevel12 = (game: Game): null => {
  if (game.spawner.executionSequence === 0) {
    if (game.spawner.roundTimer === sec(0.1)) {
      ...
      game.gameObjects.push(new MimicEnemy({ game, position: { x: 10, y: 20 } }));
  ...
  if (game.spawner.executionSequence === 2) {
    if (game.spawner.roundTimer === sec(level12Stars[0] + 1.2)) {
      ...
      game.gameObjects.push(new MimicEnemy({ game, position: { x: 10, y: 220 } }));
