...
export default class Hud {
  ...
  draw(context: any) {
    ...
    if (this.game.dev) {
      ...
      context.fillText(`Relic_id: ${relicManager.relic?.name}`, 10, 210);
      context.fillText(`uses: ${relicManager.available_uses}`, 10, 225);
    }
  ...
  roundedRectangle(context: any, x: number, y: number, width: number, height: number, rounded: number) {
